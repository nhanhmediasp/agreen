...
import { 
  ...
  BarChart2, Calendar, Receipt, Percent, Wallet,
...
/**
 * Thu nhập tài xế của một chuyến.
 *
 * Bản cũ viết `o.driverCommissionAmount || Math.round(o.totalAmount * 0.8)`:
 * vì 0 là giá trị falsy nên chuyến chiết khấu 0% bị nhảy thành 80%.
 * Dùng ?? để chỉ rơi vào giá trị mặc định khi thật sự thiếu dữ liệu.
 */
function driverEarningOf(order: ServiceOrder): number {
  if (order.driverCommissionAmount !== undefined && order.driverCommissionAmount !== null) {
    return order.driverCommissionAmount;
  }
  const rate = order.driverCommissionRate ?? DEFAULT_COMMISSION_RATE;
  return Math.round((order.totalAmount * rate) / 100);
}
...
export default function ServiceOrders() {
  ...
  const totalDriverEarnings = serviceOrders.reduce((sum, o) => sum + driverEarningOf(o), 0);
...
Cảm ơn quý khách đã sử dụng dịch vụ!`;
  ...
  const handleSaveOrder = async (e: React.FormEvent) => {
    ...
    if (!orderForm.serviceDate) {
      showToast('Vui lòng chọn thời gian chạy chuyến!', 'error');
      return;
    }

    // Km kết thúc phải >= km bắt đầu. Bản cũ dùng Math.max(0, ...) nên nhập ngược
    // sẽ âm thầm ra quãng đường 0 km và tiền cước chỉ còn phụ phí.
    const startKmNum = Number(orderForm.startKm) || 0;
    const endKmNum = Number(orderForm.endKm) || 0;
    if (endKmNum < startKmNum) {
      showToast(`Số km kết thúc (${endKmNum.toLocaleString('vi-VN')}) phải lớn hơn hoặc bằng số km bắt đầu (${startKmNum.toLocaleString('vi-VN')})!`, 'error');
      return;
    }

    let selectedDriverId = orderForm.driverId;

    // Thêm nhanh tài xế mới ngay trong form đơn
    if (isQuickCreateDriver) {
      ...
      const phone = orderForm.quickDriverPhone.trim();
      const existing = drivers.find(d => d.phone === phone);
      if (existing) {
        selectedDriverId = existing.id;
      } else {
        const newDriverObj: Driver = {
          id: '', // server sinh id
          name: orderForm.quickDriverName,
          phone,
          licenseNumber: orderForm.quickDriverLicense,
        ...
        // addDriver trả về bản ghi server đã tạo, kèm id thật
        const created = await addDriver(newDriverObj);
        if (!created) return;
        selectedDriverId = created.id;
        showToast(`Đã thêm nhanh tài xế ${created.name}`, 'success');
      }
    }

    // Quãng đường, tiền cước và chiết khấu đều do SERVER tính lại từ km & đơn giá,
    // client chỉ gửi dữ liệu thô -> hai bên không thể lệch nhau.
    const payload = {
      ...
      startKm: startKmNum,
      endKm: endKmNum,
      pricePerKm: Number(orderForm.pricePerKm) || 0,
      extraFee: Number(orderForm.extraFee) || 0,
      driverCommissionRate: Number(orderForm.driverCommissionRate),
      paymentStatus: orderForm.paymentStatus,
      notes: orderForm.notes,
    };

    if (editingOrder) {
      const ok = await updateServiceOrder(editingOrder.id, payload);
      if (!ok) return;
      showToast(`Đã cập nhật đơn dịch vụ ${editingOrder.id}`, 'success');
    } else {
      const ok = await addServiceOrder({ ...payload, status: 'completed' } as Omit<ServiceOrder, 'id' | 'createdAt'>);
      if (!ok) return;
      showToast('Tạo thành công đơn dịch vụ mới!', 'success');
    }
  ...
  const handleSaveDriver = async (e: React.FormEvent) => {
    ...
    if (editingDriver) {
      const ok = await updateDriver(editingDriver.id, {
        ...
        // Number(...) thay cho `|| 80`: chiết khấu 0% phải giữ nguyên là 0
        commissionRate: Number(driverForm.commissionRate),
      ...
      const newDriver: Driver = {
        id: '', // server sinh id, không dùng Date.now().slice(-4) vốn trùng sau mỗi 10 giây
        name: driverForm.name,
      ...
      const ok = await addDriver(newDriver);
      if (!ok) return;
  ...
  const handleDeleteOrder = async (id: string) => {
    if (!window.confirm(`Bạn có chắc chắn muốn xóa đơn dịch vụ ${id}?`)) return;
    const ok = await deleteServiceOrder(id);
    if (ok) showToast(`Đã xóa đơn dịch vụ ${id}`, 'info');
  };

  // Delete Driver
  const handleDeleteDriver = async (id: string, name: string) => {
    if (!window.confirm(`Bạn có chắc chắn muốn xóa tài xế ${name}?`)) return;
    // Server chặn nếu tài xế còn đơn dịch vụ trong lịch sử (giữ dữ liệu đối soát)
    const ok = await deleteDriver(id);
    if (ok) showToast(`Đã xóa tài xế ${name}`, 'info');
  };

  // Active driver object for dedicated Full-Page Driver Detail
  const activeDriverPageObj = selectedDriverIdPage ? drivers.find(d => d.id === selectedDriverIdPage) : null;

  // Filtered orders for Driver Detail Page
  const getOrdersForDriverDetailPage = () => {
    if (!activeDriverPageObj) return [];
    return serviceOrders.filter(o => {
      const isThisDriver = o.driverId === activeDriverPageObj.id || o.driverName === activeDriverPageObj.name;
      if (!isThisDriver) return false;
      if (!driverPageDateFilter) return true;
      const orderDateStr = o.serviceDate ? o.serviceDate.slice(0, 10) : o.createdAt.slice(0, 10);
      return orderDateStr === driverPageDateFilter;
    });
  };

  // IF USER IS VIEWING DEDICATED FULL-PAGE DRIVER DETAIL
  if (activeDriverPageObj) {
    const driverOrders = getOrdersForDriverDetailPage();
    const allDriverOrders = serviceOrders.filter(o => o.driverId === activeDriverPageObj.id || o.driverName === activeDriverPageObj.name);
    const totalRevenue = allDriverOrders.reduce((sum, o) => sum + o.totalAmount, 0);
    const totalDriverEarnings = allDriverOrders.reduce((sum, o) => sum + driverEarningOf(o), 0);
    const totalUnpaid = allDriverOrders.filter(o => o.paymentStatus === 'unpaid').reduce((sum, o) => sum + o.totalAmount, 0);
    const totalKm = allDriverOrders.reduce((sum, o) => sum + Math.max(0, o.endKm - o.startKm), 0);
    const assignedCar = cars.find(c => c.id === activeDriverPageObj.assignedCarId);
    ...
    return (
      <div className="page-container" style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          >
            <ArrowLeft size={18} />
            Quay lại Danh sách Vận hành & Tài xế
          </button>

          <div style={{ background: '#FFFFFF', padding: '24px', borderRadius: '16px', border: '1px solid #E2E8F0', boxShadow: '0 2px 6px rgba(0,0,0,0.04)', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: '#006837', color: '#FFF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', fontWeight: '800', boxShadow: '0 4px 10px rgba(0,104,55,0.25)', overflow: 'hidden' }}>
                {activeDriverPageObj.avatar ? (
                  <img src={activeDriverPageObj.avatar} alt={activeDriverPageObj.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  activeDriverPageObj.name.slice(0, 1).toUpperCase()
                )}
              </div>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ 
                    ...
                    padding: '3px 9px', 
                    borderRadius: '12px',
                    background: activeDriverPageObj.status === 'available' ? '#DCFCE7' : activeDriverPageObj.status === 'on_trip' ? '#DBEAFE' : '#F1F5F9',
                    color: activeDriverPageObj.status === 'available' ? '#15803D' : activeDriverPageObj.status === 'on_trip' ? '#1E40AF' : '#64748B'
                  }}>
                    {activeDriverPageObj.status === 'available' ? '🟢 Sẵn sàng' : activeDriverPageObj.status === 'on_trip' ? '🔵 Đang chuyến' : '⚪ Đang nghỉ'}
                  </span>
                </div>

                <div style={{ fontSize: '13.5px', color: '#64748B', display: 'flex', flexWrap: 'wrap', gap: '14px', marginTop: '6px' }}>
                  <span>SĐT: <strong style={{ color: '#1E293B' }}>{activeDriverPageObj.phone}</strong></span>
                  <span>GPLX: <strong style={{ color: '#1E293B' }}>{activeDriverPageObj.licenseNumber} (Hạng {activeDriverPageObj.licenseClass})</strong></span>
                  <span>Chiết khấu mặc định: <strong style={{ color: '#D97706' }}>{activeDriverPageObj.commissionRate || 80}%</strong></span>
                  {assignedCar && (
                    <span style={{ color: '#059669', fontWeight: 600 }}>Xe phụ trách: {assignedCar.id} ({assignedCar.name})</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <button
              onClick={() => handleOpenDriverModal(activeDriverPageObj)}
              className="btn btn-secondary"
              style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 16px', borderRadius: '10px', fontWeight: 600 }}
            >
              <Edit size={16} /> Chỉnh sửa thông tin & Ảnh
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          ...
          <div style={{ background: '#FEF3C7', border: '1px solid #FDE68A', padding: '20px', borderRadius: '14px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: '#B45309' }}>THU NHẬP TÀI XẾ NHẬN</span>
              <Wallet size={20} style={{ color: '#D97706' }} />
            </div>
            <div style={{ fontSize: '24px', fontWeight: '900', color: '#D97706', marginTop: '10px' }}>
              {totalDriverEarnings.toLocaleString('vi-VN')} đ
            </div>
            <div style={{ fontSize: '12px', color: '#B45309', marginTop: '4px' }}>Tính trên % chiết khấu từng đơn</div>
          </div>

          <div style={{ background: '#ECFDF5', border: '1px solid #A7F3D0', padding: '20px', borderRadius: '14px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: '#047857' }}>TỔNG CƯỚC THU KHÁCH</span>
              <DollarSign size={20} style={{ color: '#059669' }} />
            </div>
            <div style={{ fontSize: '24px', fontWeight: '900', color: '#006837', marginTop: '10px' }}>
              {totalRevenue.toLocaleString('vi-VN')} đ
            </div>
            <div style={{ fontSize: '12px', color: '#059669', marginTop: '4px' }}>Tổng doanh thu tiền cước</div>
          </div>

          <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', padding: '20px', borderRadius: '14px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: '#1D4ED8' }}>SỐ ĐƠN HOÀN THÀNH</span>
              <Compass size={20} style={{ color: '#2563EB' }} />
            </div>
            <div style={{ fontSize: '24px', fontWeight: '900', color: '#1E40AF', marginTop: '10px' }}>
              {allDriverOrders.length} chuyến
            </div>
            <div style={{ fontSize: '12px', color: '#2563EB', marginTop: '4px' }}>{totalKm.toLocaleString('vi-VN')} KM thực tế</div>
          </div>

          <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', padding: '20px', borderRadius: '14px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: '#B91C1C' }}>CÔNG NỢ CHƯA THU</span>
              <Clock size={20} style={{ color: '#DC2626' }} />
            </div>
            <div style={{ fontSize: '24px', fontWeight: '900', color: '#DC2626', marginTop: '10px' }}>
              {totalUnpaid.toLocaleString('vi-VN')} đ
            </div>
            <div style={{ fontSize: '12px', color: '#EF4444', marginTop: '4px' }}>
              {allDriverOrders.filter(o => o.paymentStatus === 'unpaid').length} đơn chưa thu tiền
            </div>
          </div>

        </div>

        {/* Date Filter & Table Title */}
        <div style={{ background: '#FFFFFF', padding: '16px 20px', borderRadius: '14px', border: '1px solid #E2E8F0', marginBottom: '16px', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Receipt size={22} style={{ color: '#006837' }} />
            <div>
              <h3 style={{ margin: 0, fontSize: '17px', fontWeight: 800, color: '#0F172A' }}>
                Danh Sách Hóa Đơn & Chuyến Đi Hằng Ngày
              </h3>
              <div style={{ fontSize: '12px', color: '#64748B', marginTop: '2px' }}>Chi tiết từng đơn dịch vụ của tài xế {activeDriverPageObj.name}</div>
            </div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ fontSize: '13px', fontWeight: 600, color: '#475569', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Calendar size={15} /> Lọc ngày:
            </span>
            <input
              type="date"
            ...
            {driverPageDateFilter && (
              <button
                onClick={() => setDriverPageDateFilter('')}
                style={{ padding: '7px 12px', background: '#F1F5F9', border: 'none', borderRadius: '8px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', color: '#475569' }}
              >
                Xem tất cả các ngày
              </button>
            )}
        ...
        <div style={{ background: '#FFFFFF', borderRadius: '14px', border: '1px solid #E2E8F0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          {driverOrders.length === 0 ? (
            <div style={{ padding: '48px', textAlign: 'center', color: '#64748B' }}>
              ...
              <p style={{ margin: 0, fontSize: '16px', fontWeight: 600 }}>Không tìm thấy hóa đơn nào của tài xế trong ngày đã chọn</p>
            </div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13.5px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ background: '#F8FAFC', borderBottom: '1px solid #E2E8F0', color: '#475569', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    <th style={{ padding: '12px 14px' }}>Mã Hóa Đơn / Ngày</th>
                    <th style={{ padding: '12px 14px' }}>Xe Chạy</th>
                    <th style={{ padding: '12px 14px' }}>Điểm Đón / Điểm Trả</th>
                    <th style={{ padding: '12px 14px' }}>Số KM & Đơn Giá</th>
                    <th style={{ padding: '12px 14px' }}>Tổng Cước Thu</th>
                    <th style={{ padding: '12px 14px' }}>Chiết Khấu Nhận</th>
                ...
                <tbody>
                  {driverOrders.map(order => {
                    ...
                    const commRate = order.driverCommissionRate ?? 80;
                    const commAmount = driverEarningOf(order);
                    ...
                    return (
                      <tr key={order.id} style={{ borderBottom: '1px solid #F1F5F9' }}>
                        ...
                        <td style={{ padding: '10px 12px', verticalAlign: 'top' }}>
                          <span 
                            style={{ fontWeight: 700, color: '#059669', background: '#ECFDF5', padding: '3px 8px', borderRadius: '6px', border: '1px solid #A7F3D0', display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '12px' }}
                            title={carObj?.name || order.carId}
                          >
                            <Car size={13} /> {order.carId}
                          </span>
                        </td>
                          ...
                          {(order.pickupLocation || order.dropoffLocation) ? (
                            <div>
                              {order.pickupLocation && (
                                <div style={{ fontSize: '13px', fontWeight: 500, color: '#1E293B', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                  <MapPin size={13} style={{ color: '#EF4444' }} /> {order.pickupLocation}
                                </div>
                              )}
                              {order.dropoffLocation && (
                                <div style={{ fontSize: '12px', color: '#64748B', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
                                  <ArrowRight size={12} /> {order.dropoffLocation}
                                </div>
                              )}
                            </div>
                          ) : (
                            <span style={{ color: '#94A3B8', fontStyle: 'italic' }}>Tài xế tự bắt khách</span>
                          )}
                        </td>

                        {/* Distance & Price */}
                        <td style={{ padding: '12px 14px', verticalAlign: 'top' }}>
                          <div style={{ fontWeight: 700, color: '#0F172A' }}>{dist} KM</div>
                          <div style={{ fontSize: '12px', color: '#64748B', marginTop: '2px' }}>{order.pricePerKm.toLocaleString('vi-VN')} đ/km</div>
                          {order.extraFee > 0 && <div style={{ fontSize: '11px', color: '#D97706' }}>+ {order.extraFee.toLocaleString('vi-VN')}đ phụ phí</div>}
                        </td>

                        {/* Total Fare */}
                        <td style={{ padding: '12px 14px', verticalAlign: 'top' }}>
                          <div style={{ fontWeight: 800, color: '#006837', fontSize: '15px' }}>
                        ...
                        <td style={{ padding: '12px 14px', verticalAlign: 'top' }}>
                          <div style={{ fontWeight: 800, color: '#D97706', fontSize: '15px' }}>
                            {commAmount.toLocaleString('vi-VN')} đ
                          </div>
                          <div style={{ fontSize: '11px', color: '#B45309' }}>Chiết khấu: {commRate}%</div>
                        </td>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
            {filteredDrivers.map(driver => {
              ...
              const driverEarnings = driverOrders.reduce((s, o) => s + driverEarningOf(o), 0);
