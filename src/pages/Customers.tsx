...
const Customers = () => {
  ...
  const handleCreateCustomer = async (e: React.FormEvent) => {
    ...
    if (customers.some(c => c.phone === phone)) {
      showToast(`Số điện thoại ${phone} đã có trong danh sách khách hàng!`, 'error');
      return;
    }
    ...
    const customerToAdd: Customer = {
      ...
      // Hai số này server tự tính từ bảng rentals, gửi lên chỉ để khớp kiểu
      activeRentals: 0,
      totalRentals: 0,
      status: 'verified',
    ...
    const ok = await addCustomer(customerToAdd);
    if (!ok) return;
