...
import { useApp, type Owner, type Rental } from '../context/AppContext';
...
const Owners = () => {
  ...
  /**
   * Số tiền chi trả cho chủ xe.
   * Nếu đơn chưa lưu sẵn ownerCommissionAmount thì suy ra từ TỶ LỆ RIÊNG của chủ xe đó
   * (trước đây mặc định 70% cho mọi chủ xe, làm sai số đối soát).
   */
  const payoutOfRental = (rental: Rental, rate: number) =>
    rental.ownerCommissionAmount ?? Math.round((rental.rentalFee * rate) / 100);

  // Financial direct payout calculations
  const totalGrossRevenue = ownerRentals.reduce((sum, r) => sum + r.totalAmount, 0);
  const ownerPayoutAmount = ownerRentals.reduce(
    (sum, r) => sum + payoutOfRental(r, activeOwner?.commissionRate ?? 75), 0
  );
  ...
  const handleCreateOwner = async (e: React.FormEvent) => {
    ...
    const ownerToAdd: Owner = {
      ...
      image: newImage,
      // Number() thay cho parseInt()||75 để giữ được giá trị 0%
      commissionRate: Number(newCommissionRate)
    };

    const ok = await addOwner(ownerToAdd);
    if (!ok) return;
    setShowAddForm(false);
    showToast('Đã thêm chủ xe mới!', 'success');
  ...
  const handleUpdateOwner = async (e: React.FormEvent) => {
    ...
    const ok = await updateOwner(selectedOwnerId, {
      name: editName,
  ...
  const handleCreateCommissionExpense = async () => {
    ...
    const ok = await addExpense({
      ...
      date: todayStr,
      // Ghi tên chủ xe thay vì id nội bộ để phiếu chi đọc được
      ref: `Chủ xe ${activeOwner.name}`,
      location: 'Chuyển khoản / Tiền mặt'
    });

    if (ok) showToast(`Đã tạo phiếu chi thanh toán chiết khấu ${ownerPayoutAmount.toLocaleString('vi-VN')} ₫ cho chủ xe ${activeOwner.name}!`, 'success');
  };

  const handleDeleteOwner = async () => {
    if (!selectedOwnerId) return;
    if (ownerCars.length > 0) {
      showToast(`Không thể xoá: chủ xe này còn ${ownerCars.length} xe trong hệ thống. Hãy chuyển các xe đó sang chủ xe khác trước.`, 'error');
      return;
    }
    if (!confirm('Bạn có chắc chắn muốn xóa chủ xe này khỏi hệ thống?')) return;
    // Server kiểm tra lại lần nữa nên không thể tạo ra xe "mồ côi"
    const ok = await deleteOwner(selectedOwnerId);
    if (!ok) return;
    setSelectedOwnerId(null);
    showToast('Đã xoá chủ xe khỏi hệ thống.', 'success');
  };
                ...
                {filteredOwners.length === 0 ? (
                  ...
                  (() => {
                    ...
                    return paginatedOwners.map((o, idx) => {
                      ...
                      const oPayoutTotal = oRentals.reduce((s, r) => s + payoutOfRental(r, o.commissionRate ?? 75), 0);
