...
const CreateRental = () => {
  ...
  const handleFinishRental = async () => {
    ...
    setSubmitting(true);
    try {
      // Tạo khách hàng mới nếu đang ở chế độ tạo mới.
      // Không truyền activeRentals/totalRentals nữa: server tính động từ bảng rentals,
      // trước đây truyền 1 rồi addRental cộng thêm 1 -> khách mới hiện 2 đơn.
      if (customerMode === 'create') {
        const alreadyExists = customers.some(c => c.phone === customerPhone.trim());
        if (!alreadyExists) {
          ...
          if (!created) return; // toast lỗi đã hiện ở context
        }
      }

      // id và totalAmount do SERVER sinh/tính — client không tự đặt nữa
      ...
      if (!created) return;
      showToast(`Tạo đơn thuê ${created.id} thành công!`, 'success');
      setCreatedReceiptRental(created);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleQuickAddCarSubmit = async (e: React.FormEvent) => {
    ...
    const carToAdd: Car = {
      ...
      ownerPhone: quickPhone.trim(),
      image: quickImage,
      // Để trống hạn giấy tờ thay vì bịa ngày cứng — cập nhật ở trang Quản lý Đội xe
      expiryRegistration: '',
      expiryInsurance: '',
      expiryLicense: '',
    ...
    setSubmitting(true);
    const ok = await addCar(carToAdd);
    setSubmitting(false);
    if (!ok) return;
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '24px', padding: '24px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '16px' }}>
                ...
                <button
                  ...
                  disabled={submitting}
                  style={{ padding: '12px 32px', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '8px', opacity: submitting ? 0.7 : 1 }}
                >
                  <CheckCircle size={18} /> {submitting ? 'Đang tạo đơn...' : 'Tạo đơn thuê & Giao xe'}
                </button>
              </div>
            </div>
          )}
