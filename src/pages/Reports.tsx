...
/** 'YYYY-MM-DD' theo giờ địa phương (không dùng toISOString để khỏi lệch múi giờ). */
...
const Reports = () => {
  ...
  // Mặc định 30 ngày gần nhất tính từ HÔM NAY, thay vì cắm cứng 15/06–15/07/2026
  ...
  /**
   * Khoảng thời gian đang xem — luôn neo vào ngày hiện tại.
   * Bản cũ dùng mốc cố định new Date('2026-07-15') ở 4 chỗ, nên mọi đơn phát sinh
   * sau ngày đó đều không lọt vào báo cáo "7 ngày" / "30 ngày".
   */
  const { rangeStart, rangeEnd, periodDays } = useMemo(() => {
    const end = new Date();
    ...
    if (timeRange === '7d' || timeRange === '30d') {
      ...
      return { rangeStart: start, rangeEnd: end, periodDays: days };
    }

    if (timeRange === 'quarter') {
      // Quý ĐANG diễn ra, tính từ tháng hiện tại
      const now = new Date();
      const quarterStartMonth = Math.floor(now.getMonth() / 3) * 3;
      const start = new Date(now.getFullYear(), quarterStartMonth, 1, 0, 0, 0, 0);
      const qEnd = new Date(now.getFullYear(), quarterStartMonth + 3, 0, 23, 59, 59, 999);
      const days = Math.max(1, Math.round((qEnd.getTime() - start.getTime()) / 86_400_000));
      return { rangeStart: start, rangeEnd: qEnd, periodDays: days };
    }

    // custom
    const start = new Date(`${startDate}T00:00:00`);
    const customEnd = new Date(`${endDate}T23:59:59`);
    const days = Math.max(1, Math.round((customEnd.getTime() - start.getTime()) / 86_400_000));
    return { rangeStart: start, rangeEnd: customEnd, periodDays: days };
  ...
  // Đơn ĐÃ HUỶ không được tính là doanh thu
  const filteredRentals = rentals.filter(r => r.status !== 'cancelled' && inRange(r.startDate));
  ...
  const sortedUtilization = [...cars].map(c => {
    ...
    // periodDays lấy đúng theo khoảng đang chọn (kể cả "Tùy chỉnh"),
    // trước đây mọi lựa chọn không phải 7d/30d đều bị chia cho 90 ngày.
  ...
  /**
   * Top khách hàng theo SỐ LƯỢT THUÊ TRONG KỲ đang xem.
   * Bản cũ xếp hạng theo trường tổng tích luỹ và cắt còn 3 dù tiêu đề ghi "Top 5".
   */
  const sortedTopCustomers = useMemo(() => {
    const countByPhone = new Map<string, number>();
    for (const r of filteredRentals) {
      countByPhone.set(r.customerPhone, (countByPhone.get(r.customerPhone) || 0) + 1);
    }
    return customers
      .map(c => ({ ...c, rentalsInRange: countByPhone.get(c.phone) || 0 }))
      .filter(c => c.rentalsInRange > 0)
      .sort((a, b) => b.rentalsInRange - a.rentalsInRange)
      .slice(0, 5);
  }, [customers, filteredRentals]);

  // CSS charts percentages
  // Cột biểu đồ tỷ lệ theo giá trị lớn nhất; lợi nhuận âm -> cột 0 (không ép sàn 10%
  // như bản cũ, vốn khiến khoản 0 đ và khoản lỗ trông vẫn có cột)
  const maxRevenue = Math.max(totalRevenue, totalExpenses, Math.abs(profit), 1);
  const toPercent = (value: number) => Math.min(100, Math.max(0, (value / maxRevenue) * 100));
  const revenuePercent = toPercent(totalRevenue);
  const expensesPercent = toPercent(totalExpenses);
  const profitPercent = toPercent(Math.max(0, profit));
